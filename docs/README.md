# Food Fight 26 design documentation

This directory is the product and engineering source of truth for the initial vertical slice. If code and a design document disagree, either the code is a bug or the document needs an explicit revision.

## Product and game

- [Product vision](product-vision.md)
- [Game design document](game-design-document.md)
- [Game modes](game-modes.md)
- [Items and balance](items-and-balance.md)
- [Level design](level-design.md)
- [UX and UI](ux-ui.md)
- [Accessibility](accessibility.md)

## Technology

- [Technical architecture](technical-architecture.md)
- [Networking](networking.md)
- [Backend and data](backend-and-data.md)
- [Security and anti-cheat](security-and-anti-cheat.md)
- [Performance budgets](performance-budgets.md)
- [Testing and QA](testing-qa.md)
- [Deployment and operations](deployment-and-operations.md)
- [Observability](observability.md)

## Art

- [Art direction](art-direction.md)
- [Arena art](arena-art.md)
- [Arena hero model pass](arena-hero-model-pass.md)
- [M15 production prop intake](production-prop-intake.md)
- [M16 procedural chef finish](procedural-chef-finish.md)
- [M16 character reactions](character-reactions.md)
- [M17 ambient arena life](ambient-arena-life.md)
- [M18 visual validation harness](visual-validation-harness.md)
- [M18 CI visual review captures](visual-review-captures.md)
- [M18 render resource instrumentation](render-resource-instrumentation.md)
- [Asset pipeline](asset-pipeline.md)
- [Character animation](character-animation.md)
- [Character production pass](character-production-pass.md)
- [Reactive character animation](reactive-character-animation.md)
- [Skeletal character pilot](skeletal-character-pilot.md)
- [Skeletal chef model finish](skeletal-chef-model-finish.md)
- [Environment finish](environment-finish.md)
- [Lighting and material finish](lighting-materials.md)
- [VFX and juice](vfx-juice.md)
- [Concept-art brief](concept-art/README.md)

## Planning and governance

- [Roadmap](roadmap.md)
- [Visual production roadmap](visual-production-roadmap.md)
- [Legal and IP](legal-and-ip.md)
- [Glossary](glossary.md)
- [Architecture decisions](adr/README.md)

## Document status convention

Every design document is considered **provisional** until validated by playtesting. Numerical balance values are defaults for implementation, not promises. Major decisions should receive an ADR when they materially affect build architecture, networking, asset production, or operations.
